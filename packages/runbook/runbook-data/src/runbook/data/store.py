...
from runbook.core.utils.hashing import canonical_json

DEFAULT_STORE_URI = "file:.runbook"


class BlobStore:
    """Small byte store shared by local files and S3."""
    ...
    def __init__(self, uri: str):
        ...
        if self.scheme == "file":
            ...
            self.root: Path | None = Path(path).expanduser().resolve()
            self.bucket = None
            self.prefix = ""
        elif self.scheme == "s3":
    ...
    def _key(self, key: str) -> str:
        """Validate a logical relative key and apply the store prefix."""
    ...
    def _write(self, key: str, payload: bytes, *, mode: str) -> str:
        """Write one blob and log its backend, key, size, and mutability."""
        normalized = str(PurePosixPath(key))
    ...
    def put(self, key: str, payload: bytes) -> str:
        """Write a mutable blob atomically for local stores."""
        return self._write(key, payload, mode="mutable")

    def put_immutable(self, key: str, payload: bytes) -> str:
        """Create a blob once, accepting repeats only when bytes are identical."""
        normalized = str(PurePosixPath(key))
        self._key(normalized)
        if self.exists(normalized):
            ...
            logger.debug(
                "write skipped backend={} mode=immutable key={} reason=identical",
                self.scheme,
                normalized,
            )
            return normalized
    ...
    def get(self, key: str) -> bytes:
        """Read one complete blob from the configured backend."""
        normalized = str(PurePosixPath(key))
        self._key(normalized)
        if self.scheme == "file":
            assert self.root is not None
        ...
        response = boto3.client(
            ...
            endpoint_url=os.getenv("S3_ENDPOINT_URL"),
            region_name=os.getenv("AWS_DEFAULT_REGION", "us-east-1"),
        ).get_object(Bucket=self.bucket, Key=self._key(normalized))
    ...
    def put_json(self, key: str, payload: Any) -> str:
        """Serialize JSON canonically and write it as a mutable document."""
        return self.put(key, canonical_json(payload).encode("utf-8"))

    def get_json(self, key: str) -> Any:
        """Decode one JSON blob from the store."""
        return json.loads(self.get(key).decode("utf-8"))


def open_blob_store(uri: str | None = None) -> BlobStore:
    """Open a local or S3 store using an explicit URI, env override, or S3 default."""
