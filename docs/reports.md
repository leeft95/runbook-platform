...
general template's index/header link can point to an aggregate page containing
all table plots. The monthly template's `row_plot_links` selects original
input series while applying label-column cell links to the corresponding
displayed rows; its label-column header link points to the aggregate page.
