...
def test_sqlite_0002_to_head_preserves_run_config_and_pointer_state(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        runs = connection.execute(text("SELECT run_id, status, config_hash FROM runs")).mappings().all()
    ...
    assert runs == [{"run_id": "run-demo", "status": "queued", "config_hash": "hash-demo"}]
    ...
    assert head == "0003_addressable_workers"
